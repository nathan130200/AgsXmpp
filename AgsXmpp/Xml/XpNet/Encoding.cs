...
public abstract class Encoding
{
    ...
    const byte UTF8_ENCODING = 0;
    const byte UTF16_LITTLE_ENDIAN_ENCODING = 1;
    const byte UTF16_BIG_ENDIAN_ENCODING = 2;
    const byte INTERNAL_ENCODING = 3;
    //const byte ISO8859_1_ENCODING = 4;
    //const byte ASCII_ENCODING = 5;

    public static Encoding getEncoding(byte enc)
    {
        return enc switch
        {
            UTF8_ENCODING => utf8Encoding ??= new UTF8Encoding(),
            _ => throw new NotSupportedException($"Encoding '{enc}' is not supported."),
        };
    }

    readonly int _minBPC;
    ...
    void checkCharMatches(byte[] buf, int off, char c)
    {
    ...
    TOK scanComment(byte[] buf, int off, int end, Token token)
    {
        if (off != end)
        {
    ...
    /* off points to character following "<!" */
    TOK scanDecl(byte[] buf, int off, int end, Token token)
    {
        if (off == end)
            throw new PartialTokenException();
        switch (byteType(buf, off))
        {
            case BT_MINUS:
                return scanComment(buf, off + _minBPC, end, token);
            case BT_LSQB:
                token.TokenEnd = off + _minBPC;
        ...
        while (off != end)
        {
            switch (byteType(buf, off))
            {
                case BT_PERCNT:
                    ...
                    switch (byteType(buf, off + _minBPC))
                    {
                        case BT_S:
                        case BT_CR:
                        case BT_LF:
    ...
    bool targetIsXml(byte[] buf, int off, int end)
    {
        bool upper = false;
        if (end - off != _minBPC * 3)
            return false;
    ...
    TOK scanPi(byte[] buf, int off, int end, Token token)
    {
        int target = off;
        if (off == end)
            throw new PartialTokenException();
    ...
    TOK scanCdataSection(byte[] buf, int off, int end, Token token)
    {
        /* "CDATA[".length() == 6 */
        if (end - off < 6 * _minBPC)
            throw new PartialTokenException();
        for (int i = 0; i < CDATA.Length; i++, off += _minBPC)
    ...
    /* off points to character following "</" */
    TOK scanEndTag(byte[] buf, int off, int end, Token token)
    {
        if (off == end)
            throw new PartialTokenException();
        switch (byteType(buf, off))
        ...
        while (off != end)
        {
            switch (byteType(buf, off))
            {
                ...
                case BT_LF:
                    token.NameEnd = off;
                    for (off += _minBPC; off != end; off += _minBPC)
                    {
                        switch (byteType(buf, off))
                        {
                            case BT_S:
    ...
    TOK scanHexCharRef(byte[] buf, int off, int end, Token token)
    {
        if (off != end)
        {
    ...
    /* off points to character following "&#" */
    TOK scanCharRef(byte[] buf, int off, int end, Token token)
    {
        if (off != end)
        {
            int c = byteToAscii(buf, off);
            ...
            {
                case 'x':
                    return scanHexCharRef(buf, off + _minBPC, end, token);
                case '0':
                case '1':
                case '2':
                case '3':
    ...
    TOK setRefChar(int num, Token token)
    {
        if (num < 0x10000)
        {
            switch (s_charTypeTable[num >> 8][num & 0xFF])
            {
                case BT_NONXML:
                case BT_LEAD4:
                case BT_MALFORM:
                    throw new InvalidTokenException(token.TokenEnd - _minBPC);
            }
    ...
    bool isMagicEntityRef(byte[] buf, int off, int end, Token token)
    {
        switch (byteToAscii(buf, off))
        {
            case 'a':
                if (end - off < _minBPC * 4)
                    break;
                switch (byteToAscii(buf, off + _minBPC))
                {
                    case 'm':
                        if (charMatches(buf, off + _minBPC * 2, 'p')
                            && charMatches(buf, off + _minBPC * 3, ';'))
                        {
                            token.TokenEnd = off + _minBPC * 4;
                            token.RefChar1 = '&';
                            return true;
                        }
                        break;
                    case 'p':
                        if (end - off >= _minBPC * 5
                            && charMatches(buf, off + _minBPC * 2, 'o')
                            && charMatches(buf, off + _minBPC * 3, 's')
                            && charMatches(buf, off + _minBPC * 4, ';'))
                        {
                            token.TokenEnd = off + _minBPC * 5;
                            token.RefChar1 = '\'';
                            return true;
                        }
                        break;
            ...
            case 'l':
                if (end - off >= _minBPC * 3
                    && charMatches(buf, off + _minBPC, 't')
                    && charMatches(buf, off + _minBPC * 2, ';'))
                {
                    token.TokenEnd = off + _minBPC * 3;
                    token.RefChar1 = '<';
                    return true;
                }
                break;
            ...
            case 'q':
                if (end - off >= _minBPC * 5
                    && charMatches(buf, off + _minBPC, 'u')
                    && charMatches(buf, off + _minBPC * 2, 'o')
                    && charMatches(buf, off + _minBPC * 3, 't')
                    && charMatches(buf, off + _minBPC * 4, ';'))
                {
                    token.TokenEnd = off + _minBPC * 5;
                    token.RefChar1 = '"';
    ...
    /* off points to character following "&" */
    TOK scanRef(byte[] buf, int off, int end, Token token)
    {
        if (off == end)
            throw new PartialTokenException();
        ...
        switch (byteType(buf, off))
        {
            ...
            case BT_LEAD4:
                ...
                break;
            case BT_NUM:
                return scanCharRef(buf, off + _minBPC, end, token);
            default:
                throw new InvalidTokenException(off);
        }
        while (off != end)
    ...
    /* off points to character following first character of
       attribute name */
    TOK scanAtts(int nameStart, byte[] buf, int off, int end,
                         ContentToken token)
    {
        int NameEnd = -1;
        while (off != end)
                                ...
                                case BT_AMP:
                                    {
                                        ...
                                        scanRef(buf, off + _minBPC, end, token);
                                ...
                                case BT_S:
                                    if (normalized
                                        && (off == valueStart
                                            || byteToAscii(buf, off) != ' '
                                            || (off + _minBPC != end
                                                && (byteToAscii(buf, off + _minBPC) == ' '
                                                    || byteType(buf, off + _minBPC) == open))))
                                        normalized = false;
                                    off += _minBPC;
                                    break;
                                case BT_LT:
    ...
    /* off points to character following "<" */
    TOK scanLt(byte[] buf, int off, int end, ContentToken token)
    {
        if (off == end)
            throw new PartialTokenException();
        switch (byteType(buf, off))
            ...
            case BT_EXCL:
                ...
                switch (byteType(buf, off))
                {
                    case BT_MINUS:
                        return scanComment(buf, off + _minBPC, end, token);
                    case BT_LSQB:
                        return scanCdataSection(buf, off + _minBPC, end, token);
                }
                throw new InvalidTokenException(off);
            case BT_QUEST:
                return scanPi(buf, off + _minBPC, end, token);
            case BT_SOL:
                return scanEndTag(buf, off + _minBPC, end, token);
            default:
                throw new InvalidTokenException(off);
        }
            ...
            switch (byteType(buf, off))
            {
                ...
                case BT_LF:
                    ...
                    while (true)
                    {
                        ...
                        {
                            case BT_NMSTRT:
                                return scanAtts(off, buf, off + _minBPC, end, token);
                            case BT_LEAD2:
                                if (end - off < 2)
                                    throw new PartialCharException(off);
                                if (byteType2(buf, off) != BT_NMSTRT)
                               ...
                               ContentToken token)
    {
        ...
        {
            case BT_LT:
                return scanLt(buf, off + _minBPC, end, token);
            case BT_AMP:
                return scanRef(buf, off + _minBPC, end, token);
            case BT_CR:
                off += _minBPC;
                if (off == end)
                    throw new ExtensibleTokenException(TOK.DATA_NEWLINE);
    ...
    /* off points to character following "%" */
    TOK scanPercent(byte[] buf, int off, int end, Token token)
    {
        if (off == end)
            throw new PartialTokenException();
        switch (byteType(buf, off))
    ...
    TOK scanPoundName(byte[] buf, int off, int end, Token token)
    {
        if (off == end)
            throw new PartialTokenException();
        switch (byteType(buf, off))
    ...
    TOK scanLit(int open, byte[] buf, int off, int end, Token token)
    {
        while (off != end)
        {
    ...
    /// <summary>
    /// Returns an <see cref="Encoding"/> corresponding to the specified IANA character set name. 
    /// <para>
    /// Only <c>UTF-8</c> encoding is supported as now.
    /// </para>
    /// </summary>
    public Encoding getEncoding(string name)
    {
        ...
        {
            "UTF-8" => getEncoding(UTF8_ENCODING),
            _ => throw new ArgumentException($"Invalid encoding '{name}'", nameof(name)),
        };
    }
    ...
    public TOK tokenizeProlog(byte[] buf, int off, int end, Token token)
    {
        ...
        {
            case BT_QUOT:
                return scanLit(BT_QUOT, buf, off + _minBPC, end, token);
            case BT_APOS:
                return scanLit(BT_APOS, buf, off + _minBPC, end, token);
            case BT_LT:
                {
                    off += _minBPC;
                    if (off == end)
                    ...
                    {
                        case BT_EXCL:
                            return scanDecl(buf, off + _minBPC, end, token);
                        case BT_QUEST:
                            return scanPi(buf, off + _minBPC, end, token);
                        case BT_NMSTRT:
                        case BT_LEAD2:
            ...
            case BT_LF:
                ...
                return TOK.PROLOG_S;
            case BT_PERCNT:
                return scanPercent(buf, off + _minBPC, end, token);
            case BT_COMMA:
                token.TokenEnd = off + _minBPC;
                return TOK.COMMA;
            case BT_LSQB:
            ...
            case BT_RSQB:
                ...
                if (charMatches(buf, off, ']'))
                {
                    if (off + _minBPC == end)
                        throw new PartialTokenException();
                    if (charMatches(buf, off + _minBPC, '>'))
                    {
                        token.TokenEnd = off + 2 * _minBPC;
            ...
            case BT_NUM:
                return scanPoundName(buf, off + _minBPC, end, token);
            case BT_LEAD2:
                if (end - off < 2)
                    throw new PartialCharException(off);
    ...
    {
        System.Text.StringBuilder sbuf = new();
        off += _minBPC;
        end -= _minBPC;
        for (; off != end; off += _minBPC)
        {
    ...
    {
        int len = str.Length;
        if (len * _minBPC != end - off)
            return false;
        for (int i = 0; i < len; off += _minBPC, i++)
        {
    ...
    bool isNameChar2(byte[] buf, int off)
    {
        int bt = byteType2(buf, off);
        return bt == BT_NAME || bt == BT_NMSTRT;
    }

    bool isNameChar3(byte[] buf, int off)
    {
        int bt = byteType3(buf, off);
        return bt == BT_NAME || bt == BT_NMSTRT;
    }

    bool isNameChar4(byte[] buf, int off)
    {
        int bt = byteType4(buf, off);
        return bt == BT_NAME || bt == BT_NMSTRT;
    }

    const string nameStartSingles =
"\u003a\u005f\u0386\u038c\u03da\u03dc\u03de\u03e0\u0559\u06d5\u093d\u09b2" +
"\u0a5e\u0a8d\u0abd\u0ae0\u0b3d\u0b9c\u0cde\u0e30\u0e84\u0e8a\u0e8d\u0ea5" +
"\u0ea7\u0eb0\u0ebd\u1100\u1109\u113c\u113e\u1140\u114c\u114e\u1150\u1159" +
"\u1163\u1165\u1167\u1169\u1175\u119e\u11a8\u11ab\u11ba\u11eb\u11f0\u11f9" +
...
"\u4e00\u9fa5\u3021\u3029";
    ...
    const string nameSingles =
"\u002d\u002e\u05bf\u05c4\u0670\u093c\u094d\u09bc\u09be\u09bf\u09d7\u0a02" +
"\u0a3c\u0a3e\u0a3f\u0abc\u0b3c\u0bd7\u0d57\u0e31\u0eb1\u0f35\u0f37\u0f39" +
"\u0f3e\u0f3f\u0f97\u0fb9\u20e1\u3099\u309a\u00b7\u02d0\u02d1\u0387\u0640" +
"\u0e46\u0ec6\u3005";

    const string nameRanges =
"\u0300\u0345\u0360\u0361\u0483\u0486\u0591\u05a1\u05a3\u05b9\u05bb\u05bd" +
"\u05c1\u05c2\u064b\u0652\u06d6\u06dc\u06dd\u06df\u06e0\u06e4\u06e7\u06e8" +
"\u06ea\u06ed\u0901\u0903\u093e\u094c\u0951\u0954\u0962\u0963\u0981\u0983" +
"\u09c0\u09c4\u09c7\u09c8\u09cb\u09cd\u09e2\u09e3\u0a40\u0a42\u0a47\u0a48" +
...
"\u0e50\u0e59\u0ed0\u0ed9\u0f20\u0f29\u3031\u3035\u309d\u309e\u30fc\u30fe";
    ...
    /// 
    /// </summary>
    protected static int[][] s_charTypeTable;
    ...
    static void setCharType(char c, int type)
    {
        ...
        if (s_charTypeTable[hi] == null)
        {
            s_charTypeTable[hi] = new int[256];

            for (int i = 0; i < 256; i++)
                s_charTypeTable[hi][i] = BT_OTHER;
        }

        s_charTypeTable[hi][c & 0xFF] = type;
    }
    ...
    static Encoding()
    {
        ...
        for (int i = 0; i < 256; i++)
            other[i] = BT_OTHER;
        for (int i = 0; i < 256; i++)
            if (s_charTypeTable[i] == null)
                s_charTypeTable[i] = other;
        Array.Copy(asciiTypeTable, 0, s_charTypeTable[0], 0, 128);
    }
